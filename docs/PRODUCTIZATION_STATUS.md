...
3. **未完成**：独立环境的完整外部验收、高质量模型实测、公开 GitHub Release。
...
6. **启动**：本人默认双击 Web UI 启动器；Desktop DMG 用于独立环境测试和公开分发。v0.7.1 DMG 已通过 Apple 公证并附加票据。
...
13. **DMG**：已生成 v0.7.1 Apple Silicon DMG，并完成哈希、挂载、Developer ID 签名、Apple 公证、票据附加和系统信任校验。
...
19. **当前 blockers**：独立 Apple Silicon 环境的完整验收与公开 GitHub Release。
20. **GitHub Release 尚缺**：仓库页面、Release 附件上传与项目所有者发布确认。
21. **Apple Developer**：Developer ID Application、notarytool 凭据、Hardened Runtime 签名、Apple 公证和票据附加验证均已完成。
